function byte_table()
    ...
    len8 = len >> 4
    select_in_bytes8 = Vector{UInt64}(undef, len8)
    for (i, bytes) in enumerate(Iterators.partition(Iterators.partition(select_in_bytes, 2), 8))
        byte8 = zero(UInt64)
        for (lb, hb) in Iterators.reverse(bytes)
            byte8 <<= 8
            byte8 |= (UInt8(hb) << 4) | UInt8(lb)
        end
        select_in_bytes8[i] = byte8
    end
    return Tuple(select_in_bytes8)
end
